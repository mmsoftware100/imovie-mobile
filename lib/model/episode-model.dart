import 'package:imovie/model/episode-object.dart';
import 'package:json_annotation/json_annotation.dart';
import 'package:imovie/model/file-model.dart';


/// This allows the `User` class to access private members in
/// the generated file. The value for this is *.g.dart, where
/// the star denotes the source file name.
part 'episode-model.g.dart';

/// An annotation for the code generator to know that this class needs the
/// JSON serialization logic to be generated.
/// nested object
@JsonSerializable(explicitToJson: true)
class EpisodeModel{
  int id;
  EpisodeObject episode;
  int duration;
  String episode_description;
  List<FileModel> file_model_list;

  EpisodeModel(this.id, this.episode, this.duration, this.episode_description, this.file_model_list);



  /// A necessary factory constructor for creating a new User instance
  /// from a map. Pass the map to the generated `_$UserFromJson()` constructor.
  /// The constructor is named after the source class, in this case, User.
  factory EpisodeModel.fromJson(Map<String, dynamic> json) => _$EpisodeModelFromJson(json);

  /// `toJson` is the convention for a class to declare support for serialization
  /// to JSON. The implementation simply calls the private, generated
  /// helper method `_$UserToJson`.
  Map<String, dynamic> toJson() => _$EpisodeModelToJson(this);
}